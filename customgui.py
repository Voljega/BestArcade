...
class CustomGUI():
    ...
    def __init__(self,rootFrame,setKey,scriptDir,logger,mummy) :
        ...
        self.logger = logger
        self.mummy = mummy
        self.configuration = conf.loadConf(os.path.join(self.scriptDir,utils.confDir,utils.getConfFilename(self.setKey)))
    ...
    def saveConfFile(self) :        
        ...
        shutil.copy2(os.path.join(self.scriptDir,utils.confDir,utils.getConfFilename(self.setKey)),os.path.join(self.scriptDir,utils.confDir,utils.getConfBakFilename(self.setKey)))
        confFile = open(os.path.join(self.scriptDir,utils.confDir,utils.getConfFilename(self.setKey)),"w",encoding="utf-8")
    ...
    def clickVerify(self) :
        ...
        if self.guiVars['dryRun'].get() == 1 :
            error= True
            self.logger.log('WARNING: dryRun mode is used, only csv and gamelist files will be generated, roms and images will not be copied')
        if not error :
    ...
    def clickProceed(self) :
        ...
        if result :
            ...
            sorter = BasicSorter(self.configuration,self.scriptDir,self.logger,utils.getBioses(self.setKey), self.setKey)
